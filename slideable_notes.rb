module Slideable
  ...
  HORIZONTAL_DIRS = [
    [:dx, :dy], # left            [(-7..-1).to_a, [0] ]
    [:dx, :dy], # right           [(1..7).to_a, [0]   ]
    [:dx, :dy], # up (vertical)   [[0], (1..7).to_a   ]
    [:dx, :dy]  # down (vertical) [[0], (-7..-1).to_a ]
  ].freeze
  # .freeze will cause an object to become non-mutatable 

  # DIAGONAL_DIRS stores an array of diagonal directions
  DIAGONAL_DIRS = [
    [:dx, :dy], # up + left     [ (-7..-1).to_a , (1..7).to_a   ]
    [:dx, :dy], # up + right    [ (1..7).to_a, (1..7).to_a      ]
    [:dx, :dy], # down + left   [ (-7..-1).to_a, (-7..-1).to_a  ]
    [:dx, :dy]  # down + right  [ (1..7).to_a, (-7..-1).to_a    ]
  ].freeze
